...
class counting_model(object):
    ...
    def build_model(self):
        ...
        self.pred_pprob, self.soft_pprob, self.pred_plabel, self.pred_dprob, self.pred_num, self.pred_patch_count = self.focus_network(
            self.input_Img)

        # =========density estimation loss=========
        self.density_loss = 10 * self.l1_loss(self.pred_dprob, self.input_Kmap) + self.l2_loss(self.pred_dprob,
                                                                                               self.input_Kmap)
        # =========segmentation loss=========
    ...
    def focus_network(self, inputI):
        ...
        pred_dprob = conv2d(input=pred_dprob_mul, output_chn=1, kernel_size=1, stride=1, dilation=(1, 1), use_bias=True,
                            name='pred_dprob')
    ...
    def train(self):
        ...
        for epoch in np.arange(self.epoch):
            ...
            for i_dx in rand_idx:
                ...
                batch_img, batch_dmap, batch_pmap, batch_num, batch_kmap = get_batch_patches(img_path, dmap_path,
                                                                                             pmap_path,
                                                                                             self.inputI_size,
                                                                                             self.batch_size, kmap_path)
    ...
    def test_training(self, step, log_file):
        ...
        for k in range(0, len(test_img_list)):
            ...
            predicted_label, soft_pprob, pred_plabel, pred_patch_count = self.sess.run(
                [self.pred_dprob, self.soft_pprob, self.pred_plabel, self.pred_patch_count],
