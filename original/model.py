...
class counting_model(object):
    ...
    def build_model(self):
        ...
        self.pred_pprob, self.soft_pprob, self.pred_plabel, self.pred_dprob, self.pred_num, self.pred_patch_count = self.focus_network(self.input_Img)

        # =========density estimation loss=========
        self.density_loss = 10*self.l1_loss(self.pred_dprob, self.input_Kmap)+self.l2_loss(self.pred_dprob, self.input_Kmap)
        # =========segmentation loss=========
        ...
        # =========density estimation loss=========
        self.total_loss = self.density_loss + self.segment_loss + self.global_density_loss + self.count_loss
    ...
    def focus_network(self, inputI):
        ...
        map_conv1 = conv2d(input=tf.stop_gradient(pred_dprob), output_chn=8, kernel_size=2, stride=2,
        ...
        return pred_pprob, soft_pprob, pred_plabel, pred_dprob, pred_number, pred_patch_count
    ...
    def train(self):
        ...
        for epoch in np.arange(self.epoch):
            ...
            for i_dx in rand_idx:
                ...
                batch_img, batch_dmap, batch_pmap, batch_num, batch_kmap = get_batch_patches(img_path, dmap_path, pmap_path, self.inputI_size, self.batch_size, kmap_path)
                
                _, cur_train_loss = self.sess.run([u_optimizer, self.total_loss], feed_dict={self.input_Img: batch_img,self.input_Dmap: batch_dmap,self.input_Pmap: batch_pmap,self.input_num: batch_num, self.input_Kmap: batch_kmap})
    ...
    def test_training(self, step, log_file):
        ...
        all_patch_mae = np.zeros([len(test_img_list)])
        ...
        for k in range(0, len(test_img_list)): 
            ...
            img_data, dmap_data, pmap_data, kmap_data = load_data_pairs(img_path, dmap_path, pmap_path, kmap_path)
            ...
            predicted_label, soft_pprob, pred_plabel, pred_patch_count = self.sess.run([self.pred_dprob,self.soft_pprob, self.pred_plabel, self.pred_patch_count], feed_dict={self.input_Img: img_data})
            predicted_label /= 100.0
            pred_patch_count /= 100.0
                        
            k_dice_c = self.seg_dice(pred_plabel, pmap_data)
            all_dice[k, :] = np.asarray(k_dice_c)
            all_patch_mae[k] = abs(np.sum(pred_patch_count)-np.sum(dmap_data))
        ...
        mean_patch_mae = np.mean(all_patch_mae, axis=0)
