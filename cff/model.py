...
class counting_model(object):
    ...
    def test_training(self, step, log_file, training_data=False):
        ...
        all_density_mae = np.zeros([len(test_img_list)])
        ...
        for k in range(0, len(test_img_list)):
            ...
            all_density_mae[k] = abs(np.sum(predicted_label) - np.sum(dmap_data))
        ...
        summary.value.add(tag='Density_MAE', simple_value=mean_density_mae)
