...
class counting_model(object):
	...
	self.input_Kmap = tf.placeholder(dtype=tf.float32, shape=[None, None, None], name='input_Kmap')
        ...
        self.pred_pprob, self.soft_pprob, self.pred_plabel, self.pred_kprob, self.pred_num = self.focus_network(self.input_Img)

        # =========density estimation loss=========
	map_multiplier = 0.1        
	self.density_loss = map_multiplier*self.l1_loss(self.pred_kprob, self.input_Kmap)+self.l2_loss(self.pred_kprob, self.input_Kmap)
        # =========segmentation loss=========
    ...
    def focus_network(self, inputI):
        ...
        pred_kprob = conv2d(input=pred_kprob_mul, output_chn=1, kernel_size=1, stride=1, dilation=(1,1), use_bias=True, name='pred_kprob')
	...
	#*************Count for iknn map************
        ...
        map_conv2 = conv2d(input=leaky_relu(map_conv1), output_chn=16, kernel_size=2, stride=2, dilation=(1, 1),
                           name='map2', padding='valid')
        map_conv3 = conv2d(input=leaky_relu(map_conv2), output_chn=32, kernel_size=2, stride=2, dilation=(1, 1),
                           name='map3', padding='valid')
        map_linear1 = conv2d(input=leaky_relu(map_conv3), output_chn=20, kernel_size=16, stride=1, dilation=(1, 1),
                             name='maplinear', padding='valid')
        map_count_layer = conv2d(input=leaky_relu(map_linear1), output_chn=1, kernel_size=1, stride=1, dilation=(1, 1),
    ...
    def train(self):
		...
		kmap_path = kmap_list[i_dx]
                ...
                batch_img, batch_dmap, batch_kmap, batch_pmap, batch_num = get_batch_patches(img_path, dmap_path, kmap_path, pmap_path, self.inputI_size, self.batch_size) 
                
                _, cur_train_loss = self.sess.run([u_optimizer, self.total_loss], feed_dict={self.input_Img: batch_img,self.input_Kmap: batch_kmap,self.input_Pmap: batch_pmap,self.input_num: batch_num})
